...
class Components:
    ...
    def __search_count(self):
        ...
        count.setStyleSheet('color: #aeaeae; font-size: 14px; font-weight: bold;')
        return count

    def __thumbnail_img(self):
        img = qtw.QLabel(
                pixmap=qtg.QPixmap(self.__path + 'img_placeholder.png').scaled(120, 120)
            )
    ...
    def __total_time_icon(self):
        total_time_icon = qtw.QLabel(
            icon=qtg.QIcon(
                qtg.QPixmap(self.__path + 'timer2.png')
            )
        )
