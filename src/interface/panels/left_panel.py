...
class Components:
    ...
    def __search_btn(self):
        search_btn = qtw.QPushButton(
            icon=qtg.QIcon(
                qtg.QPixmap(self.__path + 'search.png')
            )
        )
    ...
    def __search_icon_bg(self):
        search_btn_bg = qtw.QWidget()
        search_btn_bg.setStyleSheet('background-color: white;')
    ...
    def __search_filter_btn(self):
        search_filter_btn = qtw.QPushButton(
            icon=qtg.QIcon(
                qtg.QPixmap(self.__path + 'filter_icon.png')
            )
        )
    ...
    def __donate_btn(self):
        donate_btn = qtw.QPushButton(
            icon = qtg.QIcon(
                qtg.QPixmap(self.__path + 'fb_icon.png')
            )
        )
    ...
    def __drop_down_results(self):
        ...
        drop_down_results.setStyleSheet(qss.scrollbar() + """QListWidget{
                background-color: white;
                margin-left: 20px;
                font-size: 14px;
        }""")
