...
class MediaWidgets:
    ...
    def create_imgWithScale(self, img_name, w, h):
        ...
        return qtw.QLabel(pixmap=qtg.QPixmap(path + img_name).scaled(w, h))

    def create_img(self, img_name):
        """Set and return an img at default scale."""
        return qtw.QLabel(pixmap=qtg.QPixmap(path + img_name))
    ...
    def create_pushIcon(self, icon_name):
        ...
        return qtw.QPushButton(icon=qtg.QIcon(qtg.QPixmap(path + icon_name)))
